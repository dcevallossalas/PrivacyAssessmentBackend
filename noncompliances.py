...
def queryNoncompliances(apiKey, id_normative, name_normative, alias_normative, id_law, name_law, alias_law, textNormative, textLaw, n, previous_id, putText):
    ...
    instructions = "Considering your previous answer, you must now analyze the privacy legal framework " + name_law + " and the categories of subclauses of the standard " + name_normative + " in order to determine for each category" \
        "the main gaps, points, aspects, factors or arguments that are not covered by the privacy legal framework. In your explanation, you will have to refer to the items analyzed. " \
        ...
        "Of course, if your analysis refers to all subarticles of an article, then you just specify the article (not each of the subarticles). Try not to be redundant and summarize the results as much as you can so that they are easy to read and interpret." \
        "Given the text of the privacy legal framework, you must determine for each one of the " + str(n) + "categories of subclauses defined by the standard (in a maximum of two paragraphs) the reasons why the privacy legal framework does not fulfill or is not aligned in some way with each subclause." \
